/**
 * Generate demo pledge data with Date of Birth for testing Bimah BC
 * Run with: npx tsx scripts/generate-demo-data-dob.ts
 */
...
function ageToDOB(age: number): string {
  ...
  // Generate a random birthday within the year
  const birthMonth = randomInt(1, 12);
  const maxDays = new Date(birthYear, birthMonth, 0).getDate();
  const birthDay = randomInt(1, maxDays);
...
function generateCSV(numRows: number): string {
  const headers = ["Name", "DOB", "2026", "2025", "Diff"];
  const rows = [headers];
...
console.error("\nSave output to file: npx tsx scripts/generate-demo-data-dob.ts > demo-pledges-dob.csv");
